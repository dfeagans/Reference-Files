...
### Chromium ###
Install Chromium using Ubuntu Software Center. Login and sync Extensions and bookmarks. Go ahead and install Chrome if required for Netflix (Chromium doesn't use the non-open-source video codec's required for Netflix).

### Tweak Style and Background ### 
I hate thumbnails, so to make the file explorer (Nautilus) show it as default, open the file explorer then on the top-left of the banner click Files>Preferences and change "View new folders using" to "List View." Under "Appearance" change the launcher icon size to the smallest possible. That's the same place you can change the background, but the picture must be in your "Picture" directory, or whatever you specified as your "Picture" directory earlier.

### Ctrl Swap ###
Emacs used the meta (or alt) key extensively, it's alot easier to use it if you switch caps-lock with alt. Until Ubuntu 13.10, Ubuntu exposed a simple checkbox in settings to do this (They might have since added it back, so try original method below first). 

#### Original Method (Pre Ubuntu 13.10): ####
...
#### Final Method ####
To get it to work after v13.10, I had to install Gnome Tweak Tool. If you have to use the tweak tool you might as well use the dark dorian theme. Just copy it off my hard drive and put it in a .ssh hidden directory in my user directory.
...
Use the commands as follows to install:
```bash
...
### Emacs and Dotfiles for Terminal Configuration ###
...
To grab all my dotfiles, wget them from my github repository, and then follow the[symlinking instructions in the setup repository](https://github.com/dfeagans/dotfiles) for utilizing the ones I want. Comment out the lines of the .bash_profile and .bashrc I don't need on the desktop, non-server computer. 
...
This should all be in my dotfiles, but adding `alias open='gnome-open'` to your .bashrc allows you to type "open ." to explore the current directory (xdg-open and gvfs-open are alternatives. I think gvfs-open is standard). If at any point I screw up the .bashrc, just grab the one out of my [dotfiles repo](https://github.com/dfeagans/dotfiles), or for a completely blank one us /etc/skel/.bashrc, the Ubuntu back-up one.
